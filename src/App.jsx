...
// --- auth helpers (stateless, token passed explicitly) ---
...
const getPrizeIndex = (luckyNumber, numbers) => {
  ...
  for (let i = 0; i < numbers.length; i++) {
    const num = numbers[i]
    if (num === luckyNumber && num === today) {
      result.push({ index: i, type: 'both' })
    } else if (num === luckyNumber) {
      result.push({ index: i, type: 'lucky' })
    } else if (num === today) {
      result.push({ index: i, type: 'date' })
    }
  }
  return result
}
    ...
    <div className='login-page'>
      <div className='login-card'>
        ...
        <p className='login-switch'>
          {mode === 'login' ? (
            <>还没有账号？<span onClick={() => { setMode('register'); setError('') }}>去注册</span></>
          ) : (
            <>已有账号？<span onClick={() => { setMode('login'); setError('') }}>去登录</span></>
          )}
        </p>
      </div>
    </div>
...
function MainPage({ balance, scratchCards, lastSignInDate, username, onSignIn, onBuyCard, onStartScratch, onLogout, disabled = {} }) {
  ...
  return (
    <div className='main-page'>
      ...
      <div className='actions'>
        <button
          className='action-btn sign-in-btn'
          onClick={onSignIn}
        ...
        <button
          className='action-btn scratch-btn'
...
function ScratchPage({ remainingCards, onBack, onNewCard, guaranteedPrize }) {
  ...
  // Set up guaranteed prize inline (must run before card useMemo)
  if (guaranteedPrize && !guaranteedSetupDone.current) {
    const pos = Math.floor(Math.random() * 18)
    guaranteedPosRef.current = pos
    guaranteedDataRef.current = guaranteedPrize
    guaranteedSetupDone.current = true
  ...
  } else if (!guaranteedPrize) {
    guaranteedPosRef.current = -1
    guaranteedDataRef.current = null
    guaranteedSetupDone.current = false
  }
  ...
  const draw = useCallback(() => {
    ...
    intersectPoints.current.slice(Math.max(0, len - 10), len).forEach(p => {
      ...
      ctx1.beginPath()
      ctx1.arc(point.x, point.y, 12, 0, Math.PI * 2)
      ctx1.fill()
      ctx1.restore()

      const index = prizeIndex.current.findIndex(i => {
        const x = 34 + Math.trunc(i.index % 6) * 38
        const y = 312 + Math.trunc(i.index / 6) * 48
    ...
    // Check scratch percentage every 5 draws
    scratchCheckCounter.current++
    if (scratchCheckCounter.current % 5 === 0) {
  ...
  const fitText = (ctx, text, cellX, cellY, maxWidth, maxSize, color) => {
    ...
    ctx.fillText(text, cellX + 19, cellY)
    ctx.textAlign = 'start'
  }
  ...
  const card = useMemo(() => {
    ...
    image.onload = () => {
      ...
      numbers.current.forEach((num, i) => {
        ...
        const isGuaranteedPos = i === guaranteedPosRef.current
        const gp = guaranteedDataRef.current

        if (isGuaranteedPos && gp) {
          const cellWidth = 38
          fitText(ctx0, gp.number, x, y, cellWidth, 20, '#ffcc16')
          fitText(ctx0, gp.amount, x, y + 18, cellWidth, 14, '#ffcc16')
        } else {
          ctx0.fillStyle = isWinner ? '#ffcc16' : 'white'
          ctx0.font = '16px sans'
          ctx0.fillText(num.toString().padStart(2, '0'), x, y)
          const amountText = `￥${cardPrizes.current[i]}`
          ctx0.fillStyle = isWinner ? '#ffcc16' : 'white'
          ctx0.font = '12px sans'
          ctx0.fillText(amountText, x - ctx0.measureText(amountText).width / 4, y + 16)
        }
      })
    ...
    const animate = () => {
      ...
      if (intersects.length > 0 && isDown.current) {
        ...
        if (hit) {
          ...
          intersectPoints.current.push(card.worldToLocal(hit.point.clone()))
          draw()
        }
      }
  ...
  useEffect(() => {
    ...
    const touchHandler = event => {
      ...
      const r = canvasRect()
      setMouse({
        x: ((event.changedTouches[0].clientX - r.left) / r.width) * 2 - 1,
        y: ((event.changedTouches[0].clientY - r.top) / r.height) * 2 - 1
      })
    }
  ...
  useEffect(() => {
    ...
    const touchMoveHandler = event => {
      event.preventDefault()
      const ndc = getNDC(event.changedTouches[0].clientX, event.changedTouches[0].clientY)
      pointer.current.x = ndc.x
      pointer.current.y = ndc.y
    }
  ...
  const handleConfirm = useCallback(() => {
    ...
    revealed.forEach(i => {
      ...
      if (i.index === guaranteedPosRef.current && guaranteedDataRef.current) {
        const gp = guaranteedDataRef.current
        if (gp.type === 'card') cardsWon += gp.value
        else prizeWon += gp.value
      } else {
        prizeWon += cardPrizes.current[i.index]
      }
    })
    // 翻倍规则：幸运数字和日期同时出现 → 奖金×2
    if (hasLucky && hasDate && prizeWon > 0) prizeWon *= 2
    setResult({ prizeWon, cardsWon, revealed: revealed.map(r => r.index) })
  }, [])

  const handleNewCard = useCallback(() => {
    onNewCard(result?.prizeWon || 0, result?.cardsWon || 0)
  }, [result, onNewCard])

  const handleBack = useCallback(() => {
    onBack(result?.prizeWon || 0, result?.cardsWon || 0)
  }, [result, onBack])
  ...
  return (
    <div className='scratch-page'>
      <div className='scratch-top-bar'>
        ...
        {!result && (
          <button
            className='top-bar-btn confirm-btn'
        ...
        <div className='result-overlay'>
          <div className='result-card'>
            {(result.prizeWon > 0 || result.cardsWon > 0) ? (
              <>
                ...
                <p className='result-sub'>
                  揭开了 {result.revealed.length} 个中奖号码
                </p>
              </>
            ) : (
              <>
            ...
            <div className='result-actions'>
              {remainingCards > 0 && (
                <button className='result-btn primary' onClick={handleNewCard}>
                  再来一张（剩 {remainingCards} 张）
                </button>
              )}
...
function App() {
  ...
  useEffect(() => {
    ...
    if (savedToken && savedUser) {
      apiGet(AUTH_BASE + '/me', savedToken).then(data => {
        if (data.ok) {
          setUser({ username: data.username, token: savedToken })
          setState({ balance: data.balance, scratchCards: data.scratchCards, lastSignInDate: data.lastSignInDate })
        } else {
  ...
  useEffect(() => {
    if (!user) return
    apiGet(API_BASE + '/state', user.token).then(data => {
      setState(data)
      setLoading(false)
  ...
  const handleLogin = (t, username) => {
    localStorage.setItem('token', t)
    localStorage.setItem('username', username)
    setUser({ username, token: t })
  }

  const handleLogout = async () => {
    if (token) {
      try { await apiPost(AUTH_BASE + '/logout', {}, token) } catch {}
    }
    localStorage.removeItem('token')
    localStorage.removeItem('username')
    setUser(null)
  ...
  const withOp = (name, fn) => async () => {
    if (activeOps[name] || !token) return
    setActiveOps(prev => ({ ...prev, [name]: true }))
    try {
      await fn()
    } finally {
      setActiveOps(prev => ({ ...prev, [name]: false }))
    }
  }

  const handleSignIn = withOp('signin', async () => {
    const res = await apiPost(API_BASE + '/signin', {}, token)
    setState(prev => ({ ...prev, scratchCards: res.scratchCards, balance: res.balance, lastSignInDate: res.lastSignInDate }))
  })

  const handleBuyCard = withOp('buy', async () => {
    const res = await apiPost(API_BASE + '/buy', {}, token)
    setState(prev => ({ ...prev, balance: res.balance, scratchCards: res.scratchCards }))
  })

  const handleStartScratch = withOp('start', async () => {
    const res = await apiPost(API_BASE + '/start', {}, token)
    setState(prev => ({ ...prev, scratchCards: res.scratchCards }))
    setGuaranteedPrize(res.guaranteedPrize || null)
    setScratchKey(k => k + 1)
    setPage('scratch')
  })

  const handleScratchBack = async (prizeWon, cardsWon) => {
    const res = await apiPost(API_BASE + '/confirm', { prizeWon, cardsWon }, token)
    setState(prev => ({ ...prev, balance: res.balance, scratchCards: res.scratchCards }))
    setPage('main')
  }

  const handleScratchNewCard = async (prizeWon, cardsWon) => {
    const res = await apiPost(API_BASE + '/confirm', { prizeWon, cardsWon }, token)
    const res2 = await apiPost(API_BASE + '/start', {}, token)
    setState(prev => ({
      ...prev,
      balance: res.balance,
      scratchCards: res2.scratchCards,
    }))
    setGuaranteedPrize(res2.guaranteedPrize || null)
    setScratchKey(k => k + 1)
  }
  ...
  if (page === 'scratch') {
    return (
      <ScratchPage
        key={scratchKey}
        remainingCards={state.scratchCards}
        onBack={handleScratchBack}
        onNewCard={handleScratchNewCard}
